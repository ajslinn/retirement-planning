...
with st.sidebar:
    ...
    with tabs[-1]:
        ...
        s1_red = st.slider("Step 1 Red %", 0, 50, int(st.session_state.defaults.get("step1_red", 20))) / 100
...
for year in range(41):
    ...
    guaranteed = p1_sp + p1_db + p2_db

    # 2. SIPP Draw Strategy (Fixing the minus figures)
    if strat == "SIPP to Threshold":
        p1_draw = max(0, min(p1_s, BR - guaranteed))
    else:
        p1_draw = max(0, min(p1_s, PA - guaranteed)) if p1_a >= 55 else 0

    p1_s -= p1_draw
    annual_tax = calc_tax(guaranteed + p1_draw)
    
    # 3. ISA Coverage
    goal = target_spend * ((1+infl)**year)
    if p1_a >= s1_age: goal *= (1 - s1_red)
    
    net_non_isa = (guaranteed + p1_draw) - annual_tax
    isa_draw = max(0, goal - net_non_isa)
    joint_i -= isa_draw

    data_log.append({
        "Age": p1_a,
        "Fixed Pension": round(guaranteed),
        "SIPP Draw": round(p1_draw),
        "ISA Draw": round(isa_draw),
        "Total Wealth": round(p1_s + joint_i),
        "TAX_LINE": round(annual_tax)
    })

df = pd.DataFrame(data_log)

# --- 4. DISPLAY ---
st.title(f"Forecast: {strat}")

fig = go.Figure()
fig.add_trace(go.Bar(x=df['Age'], y=df['Fixed Pension'], name="Fixed Pension (SP/DB)", marker_color="#4A148C"))
fig.add_trace(go.Bar(x=df['Age'], y=df['SIPP Draw'], name="SIPP Draw", marker_color="#9C27B0"))
fig.add_trace(go.Bar(x=df['Age'], y=df['ISA Draw'], name="ISA Draw", marker_color="#1F77B4"))

# The Tax Line - Explicitly tied to the TAX_LINE key
fig.add_trace(go.Scatter(
    x=df['Age'], y=df['TAX_LINE'], name="Total Tax",
    line=dict(color='red', width=4), mode='lines+markers'
))

fig.update_layout(barmode='stack', hovermode="x unified", xaxis_title="Partner 1 Age")
