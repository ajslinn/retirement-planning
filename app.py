...
# Loop-breaker for file uploader identity
...
with st.sidebar:
    ...
    if uploaded_file is not None:
        ...
        if st.session_state.last_loaded_file_id != file_id:
            try:
                ...
                # Bridge for legacy JSON keys (p1_age_drop -> step1_age)
    ...
    with tabs[0]:
        ...
        p1_l_age = st.number_input("P1 Lump Sum Age", 50, 75, int(st.session_state.defaults.get("p1_lump_age", 55)))
    ...
    if mode == "Joint":
        with tabs[1]:
            ...
            p2_acc_age = st.number_input("P2 Access Age", 50, 75, int(st.session_state.defaults.get("p2_access_age", 55)))
            p2_l_age = st.number_input("P2 Lump Sum Age", 50, 75, int(st.session_state.defaults.get("p2_lump_age", 55)))
    ...
    else: p2_age_start, p2_sipp_init, p2_sp_amt, p2_db_in, p2_acc_age, p2_l_age = 0, 0, 0, "", 55, 55

    with tabs[-1]:
        isa_joint_init = st.number_input("Joint ISA/Savings (£)", value=float(st.session_state.defaults.get("isa_bal", 580000)))
        ...
        target_spend = st.number_input("Target Annual Spend (£)", value=float(st.session_state.defaults.get("spend", 65000)))
        st.divider()
...
for year in range(41):
    ...
    # Guaranteed Income (State Pension & DB)
    ...
    # SIPP Baseline Draw (Fill Personal Allowance)
    ...
    if strat == "SIPP to Threshold":
        for p_idx in ([1, 2] if mode=="Joint" else [1]):
            acc, age, sipp_bal = (p1_acc_age, p1_a, p1_s) if p_idx == 1 else (p2_acc_age, p2_a, p2_s)
            if age >= acc:
                ...
                # Fix for negative SIPP draws: Ensure we don't draw if already over BR
                max_draw_to_br = max(0, (BR - base) / (0.75 if ufpls else 1.0))
                gross = min(sipp_bal, max_draw_to_br)
    ...
    annual_tax_total = p1_tax + p2_tax

    data_log.append({
        "ChartAge": p1_a, 
        "State Pension": round(p1_sp + p2_sp), 
        "DB Pension": round(p1_db + p2_db), 
        ...
        "P2 Balance": round(p2_s), "P2 Tax": round(p2_tax),
        "ISA Balance": round(joint_i), "Total Wealth": round(p1_s + p2_s + joint_i),
        "TaxAmountForPlot": round(annual_tax_total)
    })
...
# Income Stacks
fig.add_trace(go.Bar(x=df['ChartAge'], y=df['State Pension'], name="State Pension", marker_color="#4A148C"))
fig.add_trace(go.Bar(x=df['ChartAge'], y=df['DB Pension'], name="DB Pension", marker_color="#388E3C"))
fig.add_trace(go.Bar(x=df['ChartAge'], y=df['SIPP Draw'], name="SIPP Draw", marker_color="#9C27B0"))
fig.add_trace(go.Bar(x=df['ChartAge'], y=df['ISA Draw'], name="ISA Draw", marker_color="#1F77B4"))

# THE TAX LINE (High Visibility Red)
fig.add_trace(go.Scatter(
    x=df['ChartAge'], 
    y=df['TaxAmountForPlot'], 
    name="Total Tax Paid", 
...
fig.update_layout(
    barmode='stack', 
    title="Annual Income Sources & Total Tax Paid",
    hovermode="x unified",
    xaxis_title="Age (Partner 1)",
    yaxis_title="Amount (£)"
)
...
st.line_chart(df.set_index("ChartAge")["Total Wealth"])

st.subheader("Yearly Breakdown")
display_cols = ["ChartAge", "State Pension", "DB Pension", "SIPP Draw", "ISA Draw", "TaxAmountForPlot", "Total Wealth"]
